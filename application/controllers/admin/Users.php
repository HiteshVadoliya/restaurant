...
class Users extends BaseController
{
        ...
        {     
            ...
            if( $user_type != '' ) {
                ...
                if( $user_type == 'admin' ) {
                    ...
                    $user_type_roleId = 3;   
                    $data['url'] = 'manager';
                } else if( $user_type == 'shiftincharge' ) {
                    $user_type_roleId = 4;   
                    $data['url'] = 'shiftincharge';
                } else if( $user_type == 'employee' ) {
                    $user_type_roleId = 5;
                    $data['url'] = 'employee';
                }
                
                $data['user_type'] = $user_type_roleId;
                $data['new_add'] = $user_type;
            }
    ...
    public function ajax_list()
    {
        ...
        if(!empty($posts))
        {
            ...
            {
                $view_url = ADMIN_LINK.$this->url.'/view/'.$post[$this->id].'/';
                $edit_url = ADMIN_LINK.$this->url.'/edit/'.$post[$this->id].'/';
                $user_type = $post['roleId'];
                // if( $user_type != '' ) {
                if( $user_type == 2 ) {
                    $user_role_Name = 'admin';
                    $view_url = ADMIN_LINK.'adminuser/view/'.$post[$this->id].'/';
                    $edit_url = ADMIN_LINK.'adminuser/edit/'.$post[$this->id].'/';
                } else if( $user_type == 3 ) {
                    $user_role_Name = 'manager';
                    $view_url = ADMIN_LINK.'manager/view/'.$post[$this->id].'/';
                    $edit_url = ADMIN_LINK.'manager/edit/'.$post[$this->id].'/';
                } else if( $user_type == 4 ) {
                    $user_role_Name = 'shiftincharge';
                    $view_url = ADMIN_LINK.'shiftincharge/view/'.$post[$this->id].'/';
                    $edit_url = ADMIN_LINK.'shiftincharge/edit/'.$post[$this->id].'/';
                } else if( $user_type == 5 ) {
                    $user_role_Name = 'employee';
                    $view_url = ADMIN_LINK.'employee/view/'.$post[$this->id];
                    $edit_url = ADMIN_LINK.'employee/edit/'.$post[$this->id];
                } 
                // }
                ...
                <a href='.$edit_url.' data-id='.$post[$this->id].' class="btn btn-sm btn-info " ><i class="fa fa-pencil"></i></a>
                <a href='.$view_url.' data-id='.$post[$this->id].' class="btn btn-sm btn-info " ><i class="fa fa-eye"></i></a>
