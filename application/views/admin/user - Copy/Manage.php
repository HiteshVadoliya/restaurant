        ...
        <div class="row">
            <div class="col-xs-12 text-right">              
                <div class="form-group">
                    <?php if(isset($new_add) && !empty($new_add)) { ?> 
                    <a class="btn btn-primary " href="<?= ADMIN_LINK.$url."/add/" ?>" ><i class="fa fa-plus"></i> Add New <?= $new_add; ?></a>
                    <?php } ?>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function () {
        $('#posts').DataTable({
           ...
           "dataType": "json",
           "type": "POST",
           "data":{  '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>', 'user_type' : '<?php echo $user_type; ?>' }
                       },
          "columns": [
                  { "data": "name" },
                  { "data": "role" },
